  ...
  return (
    <>
      <div
        className={`${
          isOpen ? "w-60" : "w-20"
        }  bg-gray-800 flex flex-col p-4 relative ${
      ...
      >
        ...
        <button
          className="absolute -right-4 top-5 bg-gray-700 text-white p-1 rounded-full shadow-lg hover:scale-110 transition"
              ...
              {isOpen &&
                ...
                link.subLinks.map((sub, index) => (
                  <Link
                    ...
                    className="flex items-center p-2 gap-5 rounded-md cursor-pointer text-sm  ml-8 my-2 hover:bg-gray-600"
                  >
                    <FaAngleDoubleRight className='w-4 h-4 text-gray-400' />{sub}
                  </Link>
                ))}
            </div>
          ))}
